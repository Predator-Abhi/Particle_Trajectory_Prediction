...
class Data_Preprocess:
    ...
    def transform_data(self, X, y, time_X=None, time_y=None, box_length=None, shear_rate=None):
        
        x_timesteps = time_X.shape[0]
        y_timesteps = time_y.shape[0]
        particles = X.shape[3]
        ...
        dX = np.zeros((X.shape[0], X.shape[1]+5, X.shape[2]-1, X.shape[3]))
        dy = np.zeros((y.shape[0], y.shape[1], y.shape[2], y.shape[3]+5))
        # Must add last location to y here
        l_0 = np.transpose(X[:, :, -1, :], (0, 2, 1))
        l_y = np.concatenate((np.expand_dims(l_0, axis=2), y), axis=2)
        
        for i in range(particles):
            for j in range(x_timesteps-1):
                dX_x_s = np.sin(2*np.pi*X[0, 0, j+1, i]/a) - np.sin(2*np.pi*X[0, 0, j, i]/a)
                dX_x_c = np.cos(2*np.pi*X[0, 0, j+1, i]/a) - np.cos(2*np.pi*X[0, 0, j, i]/a)
                dX_y_s = np.sin(2*np.pi*X[0, 1, j+1, i]/a) - np.sin(2*np.pi*X[0, 1, j, i]/a)
                dX_y_c = np.cos(2*np.pi*X[0, 1, j+1, i]/a) - np.cos(2*np.pi*X[0, 1, j, i]/a)
                
                lee_edwards_velocity = shear_rate * box_length[0]
                lee_edwards_shift = lee_edwards_velocity * time_X[j]
                lee_edwards_shift -= np.round(lee_edwards_shift/box_length[2]) * box_length[2]
                
                d = X[0, 0, j+1, i] - X[0, 0, j, i] 
                if d < -0.5*box_length[0]:
                    temp = X[0, 2, j+1, i] - lee_edwards_shift
                    dX_z_s = np.sin(2*np.pi*temp/a) - np.sin(2*np.pi*X[0, 2, j, i]/a)
                    dX_z_c = np.cos(2*np.pi*temp/a) - np.cos(2*np.pi*X[0, 2, j, i]/a)
                    shift_sin = np.sin(2*np.pi*X[0, 2, j+1, i]/a) - np.sin(2*np.pi*temp/a)
                    shift_cos = np.cos(2*np.pi*X[0, 2, j+1, i]/a) - np.cos(2*np.pi*temp/a)
                
                elif d >= 0.5*box_length[0]:
                    temp = X[0, 2, j+1, i] + lee_edwards_shift
                    dX_z_s = np.sin(2*np.pi*temp/a) - np.sin(2*np.pi*X[0, 2, j, i]/a)
                    dX_z_c = np.cos(2*np.pi*temp/a) - np.cos(2*np.pi*X[0, 2, j, i]/a)
                    shift_sin = np.sin(2*np.pi*X[0, 2, j+1, i]/a) - np.sin(2*np.pi*temp/a)
                    ...
                    dX_z_s = np.sin(2*np.pi*X[0, 2, j+1, i]/a) - np.sin(2*np.pi*X[0, 2, j, i]/a)
                    dX_z_c = np.cos(2*np.pi*X[0, 2, j+1, i]/a) - np.cos(2*np.pi*X[0, 2, j, i]/a)
                    shift_sin = 0
                    shift_cos = 0
                
                temp = np.array((dX_x_s, dX_x_c, dX_y_s, dX_y_c, dX_z_s, dX_z_c, shift_sin, shift_cos))
                dX[0, :, j, i] = temp
            
            for j in range(y_timesteps):
                dy_x_s = np.sin(2*np.pi*l_y[0, i, j+1, 0]/a) - np.sin(2*np.pi*l_y[0, i, j, 0]/a)
                dy_x_c = np.cos(2*np.pi*l_y[0, i, j+1, 0]/a) - np.cos(2*np.pi*l_y[0, i, j, 0]/a)
                dy_y_s = np.sin(2*np.pi*l_y[0, i, j+1, 1]/a) - np.sin(2*np.pi*l_y[0, i, j, 1]/a)
                dy_y_c = np.cos(2*np.pi*l_y[0, i, j+1, 1]/a) - np.cos(2*np.pi*l_y[0, i, j, 1]/a)
                
                lee_edwards_velocity = shear_rate * box_length[0]
                lee_edwards_shift = lee_edwards_velocity * time_y[j]
                lee_edwards_shift -= np.round(lee_edwards_shift/box_length[2]) * box_length[2]
                
                d = l_y[0, i, j+1, 0] - l_y[0, i, j, 0] 
                if d < -0.5*box_length[0]:
                    temp = l_y[0, i, j+1, 2] - lee_edwards_shift
                    dy_z_s = np.sin(2*np.pi*temp/a) - np.sin(2*np.pi*l_y[0, i, j, 2]/a)
                    dy_z_c = np.cos(2*np.pi*temp/a) - np.cos(2*np.pi*l_y[0, i, j, 2]/a)
                    shift_sin = np.sin(2*np.pi*l_y[0, i, j+1, 2]/a) - np.sin(2*np.pi*temp/a)
                    shift_cos = np.cos(2*np.pi*l_y[0, i, j+1, 2]/a) - np.cos(2*np.pi*temp/a)
                    ...
                    dy_z_s = np.sin(2*np.pi*l_y[0, i, j+1, 2]/a) - np.sin(2*np.pi*l_y[0, i, j, 2]/a)
                    dy_z_c = np.cos(2*np.pi*l_y[0, i, j+1, 2]/a) - np.cos(2*np.pi*l_y[0, i, j, 2]/a)
                    shift_sin = 0
                    shift_cos = 0
                
                temp = np.array((dy_x_s, dy_x_c, dy_y_s, dy_y_c, dy_z_s, dy_z_c, shift_sin, shift_cos))
                dy[0, i, j, :] = temp
                    
        #y_out = np.concatenate((y_out, dy), axis=-1)
        return dX, last_location, dy
    
    def get_added_pred(self, last_location, dy_pred):
        (n, p, t, c) = dy_pred.shape
        #last_location = np.expand_dims(last_location, axis=2)
        y_pred = torch.zeros((n, p, t, c-2))
        for i in range(t):
            temp = dy_pred[:, :, i, 4:6] + dy_pred[:, :, i, 6:]
            dy = torch.cat((dy_pred[:, :, i, :4], temp), dim=-1)
            y_pred[:, :, i, :] = dy + last_location
            last_location = y_pred[:, :, i, :].clone()
        return y_pred
