...
class Seq2Seq(nn.Module):
    ...
    def decoder_gru(self, hidden, last_location):
        ...
        for i in range(1, self.TIME_STEPS+1):
            ...
            temp = linear_out[:, 4:6] + linear_out[:, 6:]
            dy = torch.cat((linear_out[:, :4], temp), dim=1)
            
            last_location = dy + last_location
            #concatenate = torch.cat((last_location, linear_out), dim=1)
            #outputs.append(concatenate.unsqueeze(axis=1))
            outputs.append(linear_out.unsqueeze(axis=1))
