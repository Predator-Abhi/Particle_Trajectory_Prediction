...
class Model(nn.Module):
    ...
    def forward(self, x, G_fixed, last_velocity, last_location):
        assert len(last_velocity.shape) == 3
        ...
        (n, p, t, c) = x.shape
        x = x.reshape(-1, t, c)
        (n, p, c) = last_velocity.shape
        last_velocity = last_velocity.reshape(-1, c)
        (n, p, c) = last_location.shape
        last_location = last_location.reshape(-1, c)        
        
        seq_outputs = []
        for i in range(self.SEQ_HOW_MANY):
            seq_outputs.append(self.tps[i](x, last_velocity))

        dy = torch.mean(torch.stack(seq_outputs), axis=0).reshape(n, p, self.SEQ_TIMESTEPS,
                                                                 self.CONV_IN_CHANNELS)
        (n, p, t, c) = dy.shape
        dev = dy.device
        y_pred = torch.zeros((n, p, t, c), device=dev)
        for i in range(t):
            y_pred[:, :, i, :] = dy[:, :, i, :] + last_location
