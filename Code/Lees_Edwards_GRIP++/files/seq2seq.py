...
class Seq2Seq(nn.Module):
    ...
    def decoder_gru(self, hidden, last_velocity):
        assert len(last_velocity.shape) == 2
        assert len(hidden.shape) == 3 
        
        outputs = []                   
        hidden_dec_curr = hidden
        for i in range(1, self.TIME_STEPS+1):
            curr_cell_x = last_velocity
            dev = last_velocity.device
            ...
            last_velocity = linear_out + last_velocity
            outputs.append(last_velocity.unsqueeze(axis=1))
    ...
    def forward(self, in_data, last_velocity):
        hidden = self.encoder_gru(in_data)
        outputs = self.decoder_gru(hidden, last_velocity)
        return outputs