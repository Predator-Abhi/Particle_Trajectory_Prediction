...
G_fixed = dp.get_G_fixed(X, box_length, d_threshold=3*2.5e-6, alpha=0.001)
X_transformed, last_velocity, last_location, y_transformed = dp.transform_data(X, y, box_length)

batch_size = 1

print(f"X_transformed : {X_transformed.shape}, G_fixed : {G_fixed.shape}, last_velocity : {last_velocity.shape}") 
...
class Dataset_class(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        return self.x[i], self.g[i], self.v[i], self.l[i], self.y[i]

# initialize the dataset, dataloader and loss function
dataset = Dataset_class(X_transformed, G_fixed, last_velocity, last_location, y_transformed)
...
def get_train(dataloader, model, loss_fn, optimizer):
    ...
    for data in dataloader:
        x_train, g_fixed, last_velocity, last_location, y_train = data
        ...
        pred = model(x_train, g_fixed, last_velocity, last_location)
...
last_velocity_torch = torch.from_numpy(last_velocity).float().to(DEVICE)
last_location_torch = torch.from_numpy(last_location).float().to(DEVICE)
best_model = best_model.to(DEVICE)
y_pred = best_model.forward(dX_torch, G_fixed_torch, last_velocity_torch, last_location_torch)
y_pred = dp.inverse_transform(y_pred, box_length).detach().numpy()

rmse = dp.compute_rmse(y, y_pred, box_length)
