...
class Data_Preprocess:
    ...
    def extraction(self):
        ...
        return box_length, data
    
    def get_data(self, output_timesteps):
        box_length, data = self.extraction()
        t = data.shape[0]
        ...
        return X, y, box_length
    
    def get_periodic_distance(self, r_a, r_b, box_length):
    ...
    def get_G_fixed(self, X, box_length, d_threshold, alpha = 0.001):
        # Compute matrix
        ...
        for i in range(n_particles):
            for j in range(n_particles):
                r = self.get_periodic_distance(particles[i], particles[j], box_length)
    ...
    def transform_data(self, X, y, box_length=None, mode="default"):
        if mode == "default":
            dX = X[:, :, 1:, :] - X[:, :, :-1, :]
            last_location = X[:, :, -1, :] # has a shape of (n, c, p)
    ...
    def inverse_transform(self, y_pred, box_length=None, mode="default"):
        if mode == "default":
            out = y_pred
        elif mode == "sin_cos":
            a = box_length[0]
    ...
    def compute_rmse(self, y_true, y_pred, box_length):
        # y shape (n, p, t, c)
        rmse = 0
        ...
        for i in range(a):
            for j in range(c):
                for k in range(b):
                    r = self.get_periodic_distance(y_true[i, k, j, :], y_pred[i, k, j, :], box_length)
                    rmse += np.sum(r**2)
        N = a*b*c
        rmse = np.sqrt(rmse/N)
        print(f"RMSE: {rmse}")
