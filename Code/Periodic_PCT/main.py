...
mode = "sin_cos"
X, y, box_length = dp.get_data(output_timesteps=10)
G_fixed = dp.get_G_fixed(X, box_length, d_threshold=3*2.5e-6, alpha=0.001)
X_transformed, last_location, y_transformed = dp.transform_data(X, y, box_length, mode=mode)
...
# Set desired GPU
torch.cuda.set_device(2)  # Assigning second GPU
...
y_pred = best_model.forward(dX_torch, G_fixed_torch, last_location_torch)
y_pred = dp.inverse_transform(y_pred, box_length, mode=mode)
